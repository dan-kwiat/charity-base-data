...
const update = async (arr) => {
  ...
  const queries = arr.map(({ id, size, bucket, key }) =>
    knex(TABLE_SOCIAL)
      .where("regno", "=", id)
      .update({
...
const batchHandler = (items, counter) => {
  const sizedItems = items.reduce(
    (agg, { id, twitter, facebook }) => [
      ...agg,
...
const f = async () => {
  try {
    log.info(
      `Fetching avatars, persisting to S3 and saving paths to table '${TABLE_SOCIAL}'`
    )
