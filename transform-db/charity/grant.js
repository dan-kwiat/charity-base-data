...
const parser = (x) => {
  ...
  const grantsList = x.grants.sort(
    (a, b) => new Date(b.awardDate) - new Date(a.awardDate)
  )
  const fundersDict = grantsList.reduce(
    (agg, { funder }) =>
      agg[funder.id]
        ? agg
        : {
  ...
  const fundersList = Object.keys(fundersDict).map((id) => ({
    id,
    name: fundersDict[id],
  }))
  return {
...
const update = async (arr) => {
  const updateQueries = arr.map(({ chcId, funding }) =>
    knex(TABLE_CHARITY_JSON).where("chcId", "=", chcId).update({ funding })
  )
...
const f = async () => {
  try {
    log.info(
      `Persisting data from '${TABLE_GRANTNAV}' to '${TABLE_CHARITY_JSON}'`
    )

    const countQuery = knex(`${TABLE_GRANTNAV} as g`)
      .innerJoin(
        `${TABLE_MAIN_CHARITY} as mc`,
        "mc.regno",
        "=",
        "g.recipient_charity_number"
      )
      ...
      .select([
        ...
        knex.raw(`JSON_ARRAYAGG(
          JSON_OBJECT(
            ...
            'currency', g.currency,
            'funder', JSON_OBJECT('id', g.funder_id, 'name', g.funder_name)
          )
        ) as grants`),
      ])
