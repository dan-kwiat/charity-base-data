...
const scrape = async ({ id, url }) => {
  try {
    ...
    $('a[href*="instagram.com"]').each(function (i, val) {
      const handle = instagramHandle(val.attribs.href)
      if (!handle) return
      handles["instagram"][handle] = 1 + (handles["instagram"][handle] || 0)
    })
    ...
    const facebookArr = Object.keys(handles.facebook).map((x) => ({
      user: x,
      count: handles.facebook[x],
    }))
    const instagramArr = Object.keys(handles.instagram).map((x) => ({
      user: x,
      count: handles.instagram[x],
    }))
    const twitterArr = Object.keys(handles.twitter).map((x) => ({
      user: x,
      count: handles.twitter[x],
    }))

    const fb = sortHandles(facebookArr, cleanUrl)[0]
    const insta = sortHandles(instagramArr, cleanUrl)[0]
    ...
    return {
      ...
      instagram: insta ? insta.user : undefined,
    }
  } catch (e) {
    // log.error(e)
